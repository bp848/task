...
interface TodayViewProps {
  ...
  onAddRoutines: () => void;
  erpCustomers?: string[];
  erpProjects?: Array<{ name: string; customerName: string }>;
}
...
const TodayView: React.FC<TodayViewProps> = ({
  ...
  const filteredCustomerSuggestions = useMemo(() => {
    if (!customerInput.trim() || !erpCustomers?.length) return [];
    return erpCustomers
      .filter(c => c.toLowerCase().includes(customerInput.toLowerCase()))
      .slice(0, 8);
  }, [customerInput, erpCustomers]);

  const filteredProjectSuggestions = useMemo(() => {
    if (!projectInput.trim() || !erpProjects?.length) return [];
    return erpProjects
      .filter(p => {
        const nameMatch = p.name.toLowerCase().includes(projectInput.toLowerCase());
        const customerMatch = !customerInput.trim() || p.customerName.toLowerCase().includes(customerInput.toLowerCase());
        return nameMatch && customerMatch;
      })
      .map(p => p.name)
      .slice(0, 8);
  }, [projectInput, customerInput, erpProjects]);
  ...
  const handleAddTask = () => {
    ...
    onAddTask(inputValue, 'p1', [], 3600, targetDate, undefined, false, customerInput, projectInput, detailsInput || undefined);
    setInputValue('');
    setCustomerInput('');
    setProjectInput('');
    setDetailsInput('');
    ...
    <div className="flex flex-col h-full bg-zinc-50/10 overflow-y-auto items-center py-8 px-4 pb-32">
                ...
                <div className="flex space-x-4">
                  <div className="flex-1 relative">
                    ...
                    {showCustomerSuggestions && filteredCustomerSuggestions.length > 0 && (
                      <div className="absolute left-0 right-0 top-full mt-2 bg-white border-2 border-zinc-50 rounded-2xl shadow-2xl z-50 py-2 overflow-hidden animate-in fade-in slide-in-from-top-2">
                        {filteredCustomerSuggestions.map((name, i) => (
                          <button
                            key={i}
                            className="w-full text-left px-5 py-3 text-sm text-zinc-700 hover:bg-zinc-50 font-black border-b border-zinc-50 last:border-0"
                            onMouseDown={() => { setCustomerInput(name); setShowCustomerSuggestions(false); }}
                          >
                            {name}
                          </button>
                        ))}
                  ...
                  <div className="flex-1 relative">
                    ...
                    <input
                      value={projectInput}
                      onChange={(e) => { setProjectInput(e.target.value); setShowProjectSuggestions(true); }}
                    ...
                    {showProjectSuggestions && filteredProjectSuggestions.length > 0 && (
                      <div className="absolute left-0 right-0 top-full mt-2 bg-white border-2 border-zinc-50 rounded-2xl shadow-2xl z-50 py-2 overflow-hidden animate-in fade-in slide-in-from-top-2">
                        {filteredProjectSuggestions.map((name, i) => (
                          <button
                            key={i}
                            className="w-full text-left px-5 py-3 text-sm text-zinc-700 hover:bg-zinc-50 font-black border-b border-zinc-50 last:border-0"
                            onMouseDown={() => { setProjectInput(name); setShowProjectSuggestions(false); }}
                          >
                            {name}
                          </button>
                        ))}
      ...
      <div className="w-full max-w-3xl space-y-4">
        ...
        {filteredTasks.length > 0 ? filteredTasks.map(task => {
          ...
          return (
            <div key={task.id} className={`bg-white rounded-3xl border-2 p-6 flex items-center transition-all ${task.completed
