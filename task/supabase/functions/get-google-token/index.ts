...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (userRes?.error || !user) {
      return json({ error: "Invalid user session", detail: userRes?.error?.message }, 401, req);
    }
    ...
    try {
      ...
      // Delete corrupted token so user can re-authenticate
      await service.from("user_google_tokens").delete().eq("user_id", user.id);
      return json({ error: "Google token expired. Please reconnect Google.", code: "NO_REFRESH_TOKEN", reauthenticate: true }, 409, req);
    }

    // 4. Exchange refresh_token for access_token with Google
    step = "google_exchange";
    ...
    console.error(`get-google-token error at [${step}]:`, err);
    return json({ error: "Internal server error", step, detail: String(err?.message || err) }, 500, req);
  }
});
