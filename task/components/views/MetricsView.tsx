    ...
    <div className="p-8 h-full bg-zinc-50/10 overflow-y-auto custom-scrollbar">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white p-6 rounded-3xl border-2 border-zinc-50 shadow-lg shadow-zinc-800/5">
             ...
             <div className="mt-4 w-full bg-zinc-50 h-2 rounded-full overflow-hidden">
                <div className="bg-zinc-800 h-full" style={{ width: `${stats.completionRate}%` }}></div>
             </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-3xl border-2 border-zinc-50 shadow-lg shadow-zinc-800/5 p-8">
             <h3 className="text-sm font-bold text-zinc-800 mb-6 flex items-center">
                <div className="w-1.5 h-1.5 bg-zinc-800 rounded-full mr-2 shadow-sm"></div>
                PQ/MQ 達成状況
             </h3>
             <div className="space-y-6">
                <div>
                   <div className="flex justify-between text-[11px] font-bold mb-2">
                      <span className="text-zinc-400 tracking-widest uppercase">PQ (制作品質)</span>
                      <span className="text-zinc-900 font-black">実績: {businessMetricsData.monthly.pq.actual} / 目標: {businessMetricsData.monthly.pq.target}</span>
                   </div>
                   <div className="w-full bg-zinc-50 h-2 rounded-full overflow-hidden">
                      <div className="bg-zinc-800 h-full" style={{ width: `${(businessMetricsData.monthly.pq.actual / businessMetricsData.monthly.pq.target) * 100}%` }}></div>
                   </div>
                </div>
                <div>
                   <div className="flex justify-between text-[11px] font-bold mb-2">
                      <span className="text-zinc-500 tracking-widest uppercase">MQ (利益管理)</span>
                      <span className="text-zinc-800 font-black">実績: {businessMetricsData.monthly.mq.actual} / 目標: {businessMetricsData.monthly.mq.target}</span>
                   </div>
                   <div className="w-full bg-zinc-100 h-2 rounded-full overflow-hidden">
                      <div className="bg-zinc-700 h-full" style={{ width: `${(businessMetricsData.monthly.mq.actual / businessMetricsData.monthly.mq.target) * 100}%` }}></div>
                   </div>
                </div>
             </div>
          </div>

          <div className="bg-zinc-950 rounded-3xl p-8 flex flex-col items-center justify-center text-center shadow-xl shadow-zinc-200">
             <div className="w-16 h-16 bg-white/10 text-zinc-400 rounded-full flex items-center justify-center mb-4">
             ...
             <p className="text-xs text-zinc-100/70 leading-relaxed max-w-sm font-medium">
               現在のタスク完了ペースに基づくと、午後の制作集中時間を15%増やすことで、PQ目標を予定より3日早く達成できる可能性があります。
             </p>
          </div>
        </div>

        <div className="mt-12 bg-white rounded-3xl p-8 border-2 border-zinc-50 shadow-lg shadow-zinc-800/5">
              ...
              {Array.from({ length: 52 }).map((_, i) => (
                <div key={i} className="flex flex-col space-y-1 shrink-0">
                  {Array.from({ length: 7 }).map((_, j) => {
                    const opacity = Math.random() > 0.7 ? 'bg-zinc-400' : Math.random() > 0.4 ? 'bg-zinc-800' : 'bg-zinc-50';
           ...
           <div className="mt-4 flex items-center space-x-4 text-[9px] font-black text-zinc-200 uppercase tracking-widest">
              ...
              <div className="flex space-x-1">
                <div className="w-2 h-2 bg-zinc-50 rounded-sm"></div>
