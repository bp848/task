...
export function useZenworkTasks(session: Session | null) {
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'INSERT') {
          ...
          setTasks(prev => prev.map(t => {
            ...
            // タイマー実行中はRealtimeからのtimeSpent上書きを防ぐ
            if (t.timerStartedAt && !t.timerStoppedAt) {
              return { ...updated, timeSpent: t.timeSpent };
            }
            return updated;
          }));
  ...
  const mergeCalendarTasks = useCallback((calendarTasks: Task[]) => {
    setTasks(prev => {
      const nonCalendarTasks = prev.filter(t => !t.tags.includes('Google Calendar'));
